...
def CreateLog(FolderName):
    ...
    fobj.write("CPU Usage : %s %%\n" %psutil.cpu_percent()) # error yet hota tyamule  %% kel as
    ...
    net = psutil.net_io_counters() #kiti data patavala aheani kiti milavala  ata paryan t computer suru jhalyapasun tyala count kart 
    fobj.write("\nNetwrk Usage Report\n")
    ...
    for info in Data:
        fobj.write("PID : %s\n" %info.get("pid"))#write function cha ithe colon use kart nahit
        fobj.write("Name %s\n" %info.get("name"))
        fobj.write("Username %s\n" %info.get("username"))
        fobj.write("Status : %s\n" %info.get("status"))
        fobj.write("Start time : %s\n" %info.get("create_time"))
        fobj.write("CPU %% : %.2f\n" %info.get("cpu_percent")) # after decimal point  it show  only two digit
...
def ProcessScan():
    ...
    for proc in psutil.process_iter():    # this can work to give the list of currently process   
        try:
            ...
            info["cpu_percent"] = proc.cpu_percent(None)   #None mule  magil call nantaracha cpu count mojate
            info["memory_percent"] = proc.memory_percent()

            listprocess.append(info)
        except (psutil.NoSuchProcess, psutil.AccessDenied, psutil.ZombieProcess):  #Zombie = process meleli ahe pn  ajun list madhe dia=st ahe
            pass
