...
import schedule
import shutil
import hashlib  #hashlib is used to  create unique  digitalfingerprint of data for checking safety duplication and changes

def calculate_hash(path):
    hobj = hashlib.md5() #this creates md5 hash object .md5 is mathematical algorithm that converts data  into a fixed size string
    fobj =open(path,"rb") #read binary It will work for all files

    while True:
        data = fobj.read(1024)
        if not data:
            break
        else:          #create digital fingerprint
            hobj.update(data)

    fobj.close()
    return hobj.hexdigest() #this convert hash into readable hexadecimal string
